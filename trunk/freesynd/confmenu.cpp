...
class CommonConfSubMenu : public Menu {
  public:
    CommonConfSubMenu(MenuManager * m, const char *menu_name, uint8 * bkg,
                      ConfMenu * confMenu):Menu(m, menu_name, "", ""),
        bkg_(bkg) {
        setParentMenu("conf");
...
class ChangeLogoMenu : public CommonConfSubMenu {
  public:
    ChangeLogoMenu(MenuManager * m, uint8 * bkg,
                   ConfMenu * confMenu):CommonConfSubMenu(m, "changeLogo",
                                                          bkg, confMenu),
        logo_(g_App.logo()), colour_(0) {
    ...
    virtual void handleOption(Key key) {
        ...
        if (key == KEY_F2) {
            if ((unsigned int) colour_ <
                sizeof(g_Colours) / sizeof(int) - 1)
                colour_++;
            else
                colour_ = 0;
            g_Screen.drawLogo(336, 55, logo_, g_Colours[colour_]);
...
class ChangeNameMenu : public CommonConfSubMenu {
  public:
    ChangeNameMenu(MenuManager * m, const char *menu_name, uint8 * bkg,
                   ConfMenu * confMenu):CommonConfSubMenu(m, menu_name,
                                                          bkg, confMenu),
        tick_count_(0), light_last_tick_(false) {
...
class ChangeCompanyNameMenu : public ChangeNameMenu {
  public:
    ChangeCompanyNameMenu(MenuManager * m, uint8 * bkg,
                          ConfMenu * confMenu):ChangeNameMenu(m,
                                                              "changeCompany",
                                                              bkg,
                                                              confMenu) {
...
class ChangeYourNameMenu : public ChangeNameMenu {
  public:
    ChangeYourNameMenu(MenuManager * m, uint8 * bkg,
                       ConfMenu * confMenu):ChangeNameMenu(m, "changeName",
                                                           bkg, confMenu) {
        setClearArea(bkg, 283, 28, 328, 120);
...
ConfMenu::ConfMenu(MenuManager * m):Menu(m, "conf", "mconfup.dat",
     "mconfout.dat")
{
    int size;
